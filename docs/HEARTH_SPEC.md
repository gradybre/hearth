...
- **food** — id, household_id (nullable = global), name, brand, store_tag, barcode, serving_options[], macros (kcal, protein_g, carb_g, fat_g per serving), macros_overridden (bool), mass_display_mode (automatic / ounces / weight), package_nutrition (nullable reviewed package/serving relationship), source (OFF / USDA / manual), is_deleted (soft delete)
- **personal_food** — user or household scoped foods created via manual entry / failed barcode
- **meal_plan_day** — id, user_id, date, notes
- **meal_plan_entry** — id, meal_plan_day_id, meal_slot (breakfast/lunch/dinner/snack), ref_type (food/recipe), ref_id, servings, serving_option_id (optional selected food serving; otherwise default), is_planned, is_logged, logged_at, **macro_snapshot** (kcal/protein/carb/fat + portion frozen at log time)
...
- **Package-aware mass display.** Automatic preserves ounce package evidence and authored ounces: two 12 oz bags read as 24 oz, and two 28 oz cans read as 56 oz. A food can explicitly choose Ounces or Weight (oz/lb); weight ingredients authored with pounds aggregate normally. Display choices never round or rewrite canonical amounts. Package and serving labels retain their authored units. Shopping keeps original source amounts and recalculates derived totals against the current matched food.
- **Package nutrition.** A reviewed mass package amount, selected volume serving and positive servings-per-package count establish a food-specific equivalence: 10 oz / 2 one-cup servings means 30 oz is six nutrition servings. Direct nutrition servings and the food's own usable density take precedence. Missing data never creates a universal cup-to-ounce factor. Changing the package or selected serving makes the relationship stale until reviewed; logged history stays frozen. Approximate printed counts remain qualified in nutrition and new log snapshots.
- **Manual and photo entry.** Package amount and servings per package are editable independently of Walmart. The label sheet accepts a nutrition-panel photo, a package-front photo, or both, with previews and one Read photos request. Extracted facts are proposals; conflicts and prepared/drained uncertainty remain visible and recorded. Confirm package servings freezes the reviewed relationship; Save commits the food. Partial optional package fields do not block saving unrelated food details.
- **Planned food serving basis.** A package-derived planned food retains its selected nutrition serving ID, so its count, live calorie projection, shopping amount and later log agree. An absent selected serving is unresolved rather than silently substituted. Legacy entries continue to count the default serving; logged snapshots remain authoritative.
...
- **Eating out.** A meal from a restaurant is a recipe like any other, with a `kind` of `eaten_out`: it never reaches the shopping list, has no cook-along and is not scaled. Its components are **global restaurant foods** — the chain's own published numbers, world-readable, written server-side only — kept in the sections and the order the chain prints them, so the builder lays a menu out the way the restaurant does. A restaurant's foods and a household's own are disjoint sets for matching: a cooked recipe is never auto-matched to a chain's component, and an eaten-out one is never auto-matched to a pantry item.
  - **Menus are seeded or imported, never typed.** A published nutrition table arrives as pasted text, as pictures of the page, or as a seed migration. Everything read passes through the same live review before anything is saved (§5.3's rule, and §5.7's).
    - **A guide is read in batches, and never called more than it is.** Every page rendered is an image sent to the model, so a long PDF is read a few pages at a time rather than whole — that is a cost decision and a sound one. What it must never become is a *silent* one: choosing the file says how many pages it has, each read says which pages it looked at, and the pages nobody has paid for yet are offered rather than discarded. Six pages of eighteen is a fine offer. Six pages of eighteen described as "the menu" is a wrong answer with no way to notice, and the person reading it has no reason to go looking for a section they were never told existed.
    - **A page that will not render is named.** It is not dropped from the batch, and it does not count as read: it is said out loud, by number, and offered again on the next read. A guide missing a page looks exactly like a guide that never had one.
    - **Choosing costs nothing and changes nothing.** Opening the file dialog and backing out of it is not an error and must not disturb what is already on the review screen — a doubt belongs to the rows it arrived with, and clearing it on the way to a picker leaves rows standing with their warnings gone.
...
- **Log in the food's own servings.** A food knows several — "170 g pot",
  ...
  - A serving that cannot be written as a multiple of the default one — a
    volume against a mass, with no density the food carries — is **not
    offered**, rather than offered and mis-stored (§5.5: a figure nobody
    stated is not invented). Recipes stay servings-based.
...
- **Minor nutrients (fibre, sodium, cholesterol).** *Lifted from §12 at Brendan's request, v0.9.* Three optional nutrients carried alongside the four macros, and unlike them **nullable — null means unknown, never zero**. Every source Hearth already reads gives all three (Open Food Facts and USDA return them in the payload the adapter is already fetching; a US nutrition label is legally required to print them), so they cost no extra call.
  - **Optional targets, and not members of `MacroKind`.** *Amended v1.1 — the original text said "never a target", which stopped being true at schema v22.* `macro_targets` carries a nullable `fiber_g`, `sodium_mg` and `cholesterol_mg`, and the day and week screens show progress against them. They stay out of `MacroKind`, which drives the four macros' own progress, because these three do not behave like those four: **fibre is a floor to reach, sodium and cholesterol are ceilings to stay under**, and a screen treating all three alike would be wrong about two of them. Calories stay the primary focus and the four macros stay secondary; these sit below both.
...
- **A write the queue cannot send fails; it never stops the queue.** Payloads
  outlive the build that wrote them, so a delete queued yesterday can be
  missing a column today's filter names. That write is refused as an
  `Exception` and recorded against itself — never thrown as an `Error`, which
  escapes the push loop unmarked, is retried first on every later pass, and
  takes the pull down with it. One unsendable write must cost one write.
...
- **A recovery link opens a session, not a form.** Supabase signs the user in with it, so from the app's side it is indistinguishable from an ordinary sign-in — which means without a gate somebody who came to change their password ends up in their meal plan, holding a session minted by an email, with the thing they came for three taps away. Until the password is actually set, the only screen that session may reach is the one that sets it. Leaving without setting one signs the session out: a recovery session nobody is going to use for recovery is not a session anybody asked for.
- **The redirect is opt-in, and both halves move together.** A `redirectTo` the project has not been told to allow produces a link that looks right and goes nowhere, which is worse than one that plainly goes to the project's own page. So the app sends none unless it is configured, and `docs/SUPABASE_SETUP.md` names the dashboard entry, the config value, which platforms register the scheme, and what has not been verified — Windows has no handler, and cross-device recovery is not proven by same-device recovery working.
- **The password policy belongs to the project.** The app checks length and that the two fields agree, because that saves a round trip; everything else is the server's to enforce and is shown as the server says it. A second copy of the rule here is a second copy that eventually disagrees with the one being applied.
- **A pasted link is a request made from the server, not by the phone.** The Edge Function holds the secret key and sits inside the project's network, so `fetch(whatever_you_pasted)` is an offer to reach anything the *function* can reach — `169.254.169.254` is the cloud metadata service and `127.0.0.1:54321` is PostgREST. Every URL is therefore checked before a connection is opened, and checked again at every redirect: http and https only, no credentials in the URL, and no address that is loopback, private, link-local, carrier-NAT, multicast or reserved — judged on the parsed octets, because `127.0.0.1` and `::ffff:127.0.0.1` are one address with several names. A hostname is resolved and **every** answer judged, since a name is only as safe as its worst record.
- **What comes back is bounded as it arrives.** A streaming byte cap, not `content-length` — that is the server's claim about itself, and a chunked response makes none — and a content type a recipe could plausibly be written in. Reading the whole body and *then* trimming it means a two-gigabyte answer is two gigabytes of the function's memory before the limit is consulted.
- **The residual risk is named rather than implied.** The hostname is resolved once here and again by `fetch`, so a record with a one-second lifetime can differ between the two. Closing that needs the connection pinned to an address already checked, which Deno's `fetch` does not expose. What is closed is the whole direct case, which is every version of this anybody would actually paste.
