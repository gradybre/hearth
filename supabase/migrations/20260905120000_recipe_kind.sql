...
-- own tables, policies, sync and export.
--
-- A column and not a tag, because `ShoppingListBuilder` uses it to decide
-- what goes on a shopping list, and a rule that decides what you are sent to
-- the shop for should not hang off a string somebody could rename.
--
-- Additive and defaulted: every recipe written before today is one you cook,
...
alter table public.recipes
  add column if not exists kind text not null default 'cooked'
    check (kind in ('cooked', 'eaten_out'));

comment on column public.recipes.kind is
  'cooked, or eaten_out for a restaurant meal — which never reaches the '
  'shopping list and has no cook-along or scaling (spec §5.2, §5.7).';
...
-- Last defined entire in 20260831180000_no_match_needed.sql.
...
begin
  insert into public.recipes (
    ...
    tags, kind, source, photo_url, notes, created_by, is_deleted, updated_at
  )
  values (
    v_recipe_id,
